...
public class OBJ_StoreDecoration1 extends SuperObject {
    private int decorationWidth = 3;  // 3x2 size
    private int decorationHeight = 2;
    
    public OBJ_StoreDecoration1(GamePanel gp, int col, int row) {
        super(gp, col, row);
        setName("store_decoration1");
        getPosition().setCollision(true); // Store decoration has collision
        
        try {
            BufferedImage image = ImageIO.read(getClass().getResourceAsStream("/RES/OBJECT/storedecoration1.png"));
            if (image == null) {
                image = ImageIO.read(new File("RES/OBJECT/storedecoration1.png"));
            }
            setImage(image);
            System.out.println("Store Decoration 1 image loaded successfully");
        } catch (IOException | IllegalArgumentException e) {
            System.err.println("Could not load store decoration 1 image: " + e.getMessage());
