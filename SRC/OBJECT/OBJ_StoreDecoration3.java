...
public class OBJ_StoreDecoration3 extends SuperObject {
    private int decorationWidth = 3;  // 3x1 size
    private int decorationHeight = 1;
    
    public OBJ_StoreDecoration3(GamePanel gp, int col, int row) {
        super(gp, col, row);
        setName("store_decoration3");
        getPosition().setCollision(true); // Store decoration has collision
        
        try {
            BufferedImage image = ImageIO.read(getClass().getResourceAsStream("/RES/OBJECT/storedecoration3.png"));
            if (image == null) {
                image = ImageIO.read(new File("RES/OBJECT/storedecoration3.png"));
            }
            setImage(image);
            System.out.println("Store Decoration 3 image loaded successfully");
        } catch (IOException | IllegalArgumentException e) {
            System.err.println("Could not load store decoration 3 image: " + e.getMessage());
