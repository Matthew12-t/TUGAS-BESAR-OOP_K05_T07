...
public class OBJ_StoreDecoration2 extends SuperObject {
    private int decorationWidth = 1;  // 1x5 size
    private int decorationHeight = 5;
    
    public OBJ_StoreDecoration2(GamePanel gp, int col, int row) {
        super(gp, col, row);
        setName("store_decoration2");
        getPosition().setCollision(true); // Store decoration has collision
        
        try {
            BufferedImage image = ImageIO.read(getClass().getResourceAsStream("/RES/OBJECT/storedecoration2.png"));
            if (image == null) {
                image = ImageIO.read(new File("RES/OBJECT/storedecoration2.png"));
            }
            setImage(image);
            System.out.println("Store Decoration 2 image loaded successfully");
        } catch (IOException | IllegalArgumentException e) {
            System.err.println("Could not load store decoration 2 image: " + e.getMessage());
